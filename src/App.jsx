...
const SearchableSelect = ({ options, value, onChange, placeholder, isDark }) => {
  ...
  return (
    <div className="relative" ref={wrapperRef}>
      <div 
        className={`flex items-center justify-between w-full p-2.5 rounded-lg cursor-pointer text-sm border ${
          isDark 
      ...
      {isOpen && (
        <div className={`absolute z-50 w-full mt-1 rounded-lg shadow-xl max-h-60 flex flex-col overflow-hidden border ${
          ...
          <div className={`p-2 border-b ${isDark ? 'border-slate-700' : 'border-slate-200'}`}>
            <input
              type="text"
              autoFocus
              className={`w-full p-2 rounded-md focus:outline-none focus:ring-1 focus:ring-indigo-500 text-sm border ${
          ...
          <div className="overflow-y-auto">
            {filteredOptions.length === 0 ? (
              ...
              filteredOptions.map((opt, idx) => (
                <div
                  key={idx}
                  className={`px-3 py-2.5 text-sm cursor-pointer flex justify-between items-center ${
        ...
        {data.map((item, idx) => {
          ...
          return (
            <div key={idx} className="flex items-center gap-3">
              ...
              {/* O Container do gráfico tem tamanho fixo para não distorcer com o flexbox */}
              <div className="w-2/3 flex items-center">
                <div className={`flex-1 h-6 rounded-md relative ${isDark ? 'bg-slate-700/50' : 'bg-slate-100'}`}>
                  {/* A barra colorida fica absoluta dentro do container, impedindo que ela sofra "shrink" (esmagamento) do flexbox */}
                  <div 
                ...
                {/* O Label do valor tem tamanho fixo (w-20) para o gráfico alinhar perfeito */}
                <span className={`ml-3 text-sm font-mono whitespace-nowrap w-20 text-right ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
                ...
                <div className={`flex gap-2 p-1.5 rounded-full border ${isDark ? 'bg-[#0b1120] border-slate-800' : 'bg-slate-100 border-slate-200'}`}>
                  {[
                    ...
                    <button
                      ...
                      className={`flex-1 flex items-center justify-center gap-2 px-3 py-1.5 rounded-full text-xs font-medium transition-all ${
                ...
                {showCustomForm && (
                  <div className={`p-4 rounded-lg border space-y-3 mb-4 ${isDark ? 'bg-slate-800/50 border-indigo-500/30' : 'bg-indigo-50 border-indigo-200'}`}>
                    ...
                    <button 
                      ...
                      className="w-full py-2 bg-indigo-600/80 hover:bg-indigo-600 text-white text-xs font-bold rounded flex items-center justify-center gap-2"
                    >
                          ...
                          <tr key={item.id} className={`transition-colors ${isDark ? 'hover:bg-slate-800/50' : 'hover:bg-slate-50'}`}>
                            <td className="p-3">
                              <div className={`font-semibold text-xs ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>
                                {item.name} {item.selectedVram && <span className="opacity-70 font-normal">({item.selectedVram}GB)</span>}
                              </div>
          ...
          <div className="lg:col-span-7 space-y-4">
            {selectedItems.length === 0 ? (
              <div className={`h-full min-h-[300px] flex flex-col items-center justify-center rounded-xl border border-dashed p-8 text-center ${isDark ? 'bg-[#0f172a] border-slate-700 text-slate-500' : 'bg-white border-slate-300 text-slate-400'}`}>
                ...
                <p className="text-xs mt-1">Selecione componentes ao lado para gerar comparações.</p>
              </div>
            ) : (
              <>
